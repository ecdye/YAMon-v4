...
CurrentConnections_1() { # _doCurrConnections=1
	...
	ip4t=$(iptables -L "$YAMON_IPTABLES" -vnx | grep -v RETURN | awk '{ print $2,$8,$9 }' | grep "^[1-9]")
	ip6t="$($ip6tablesFn)"
	ipt="$ip4t\n$ip6t"
	macIP="$(cat "$macIPFile")"
	...
	while true; do
		...
		if [ "$_generic_ipv4" == "$ip" ] || [ "$_generic_ipv6" == "$ip" ]; then
			...
			tip="\b${ip//\./\\.}\b"
			do="$(echo "$ipt" | grep -E "($_generic_ipv4|$_generic_ipv6) $tip\b" | cut -d' ' -f1)"
			up="$(echo "$ipt" | grep -E "$tip ($_generic_ipv4|$_generic_ipv6)" | cut -d' ' -f1)"
